...
class DBController { 
    ...
    private $user = "root";
    private $password = "";
    private $database = "agro_student_market";
    private $admin_conn;
    
    function __construct() {
        $this->admin_conn = mysqli_connect($this->host, $this->user, $this->password, $this->database);
    }

    public static function getConnection() {
        if (empty($this->admin_conn)) {
            new Database();
        }
    }

    function getDBResult($query, $params = array()) {
        $sql_statement = $this->admin_conn->prepare($query);
